...
(async () => {
    ...
    console.log("Starting in 3 seconds... Make sure you're on your saved search page.");
    ...
    function getTotalCount() {
        const el = document.querySelector('[data-testid="total-count"], .total-count, .results-count');
        if (el) {
    ...
    async function selectAllOnPage() {
        ...
        const selectAll = document.querySelector('input[type="checkbox"][aria-label*="select"], input[type="checkbox"][data-testid*="select-all"]');
        ...
        const selectAllLink = Array.from(document.querySelectorAll('a, button, span')).find(el => 
            el.textContent.match(/select all \d/i) || el.textContent.match(/select all contacts/i)
        );
        if (selectAllLink) {
            click(selectAllLink);
            await wait(500);
    ...
    async function clickExport() {
        ...
        const exportBtn = Array.from(document.querySelectorAll('button')).find(el => 
        ...
        if (exportBtn) {
            ...
            // Look for CSV option
            const csvOption = Array.from(document.querySelectorAll('button, a, li, div')).find(el => 
            ...
            const confirmBtn = Array.from(document.querySelectorAll('button')).find(el => 
                el.textContent.trim() === 'Export' || el.textContent.includes('Confirm') || el.textContent.includes('Download')
            );
            if (confirmBtn) {
                click(confirmBtn);
                await wait(3000);
    ...
    async function nextPage() {
        ...
        const nextBtn = document.querySelector('button[aria-label="Next"], button[aria-label*="next"], [data-testid*="next"]');
        ...
        const pageLinks = Array.from(document.querySelectorAll('a, button')).filter(el => {
    ...
    while (iteration < maxIterations) {
        ...
        if (exported) {
            ...
            console.log(`  Exported batch ${iteration}. Total so far: ~${totalExported}`);
        }
