...
func HandleListProjects_keboola_mcp_server(ctx context.Context, args map[string]interface{}) (ToolResponse, error) {
...
func HandleGetProject_keboola_mcp_server(ctx context.Context, args map[string]interface{}) (ToolResponse, error) {
	token, _ :=getString(args, "token")
	if token == "" {
		return err("missing token")
}
