...
func HandleConvert_currency_converter_ai_mcp(ctx context.Context, args map[string]interface{}) (ToolResponse, error) {
...
func HandleGetRate_currency_converter_ai_mcp(ctx context.Context, args map[string]interface{}) (ToolResponse, error) {
	from, _ :=getString(args, "from")
	to, _ :=getString(args, "to")
	rate, e := getExchangeRate(from, to)
	if e != nil {
