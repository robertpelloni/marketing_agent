...
func HandleGetQuote_mcp_yfinance_server(ctx context.Context, args map[string]interface{}) (ToolResponse, error) {
...
func HandleGetInfo_mcp_yfinance_server(ctx context.Context, args map[string]interface{}) (ToolResponse, error) {
	symbol, _ :=getString(args, "symbol")
	if symbol == "" {
		return err("symbol is required")
}
