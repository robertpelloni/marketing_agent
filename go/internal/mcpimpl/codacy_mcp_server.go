...
func HandleListRepositories_codacy_mcp_server(ctx context.Context, args map[string]interface{}) (ToolResponse, error) {
...
func HandleGetCoverage_codacy_mcp_server(ctx context.Context, args map[string]interface{}) (ToolResponse, error) {
	apiKey := os.Getenv("CODACY_API_TOKEN")
	if apiKey == "" {
		return err("CODACY_API_TOKEN not set")
}
