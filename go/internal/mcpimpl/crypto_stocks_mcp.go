...
func HandleGetStockPrice_crypto_stocks_mcp(ctx context.Context, args map[string]interface{}) (ToolResponse, error) {
...
func HandleGetCryptoPrice_crypto_stocks_mcp(ctx context.Context, args map[string]interface{}) (ToolResponse, error) {
	symbol, _ :=getString(args, "symbol")
	if symbol == "" {
		return err("symbol is required")
}
