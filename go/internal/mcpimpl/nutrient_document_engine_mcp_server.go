...
func HandleConvertDocument_nutrient_document_engine_mcp_server(ctx context.Context, args map[string]interface{}) (ToolResponse, error) {
...
func HandleExtractText_nutrient_document_engine_mcp_server(ctx context.Context, args map[string]interface{}) (ToolResponse, error) {
	fileURL, _ :=getString(args, "file_url")
	apiURL, _ :=getString(args, "api_url")
	if fileURL == "" {
		return err("file_url is required")
