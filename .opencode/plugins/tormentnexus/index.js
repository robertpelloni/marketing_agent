...
async function expandMemoryKeys(text) {
  ...
  for (const key of keys) {
    try {
      const data = await tnJson(`/api/memory/search?q=${encodeURIComponent(key)}&limit=2`);
      const memories = Array.isArray(data) ? data : data.memories ?? [];
      if (memories.length > 0) {
        const ctx = memories
          .map((m) => `  - ${(m.content ?? JSON.stringify(m)).slice(0, 200)}`)
          .join("\n");
        result = result.replace(`@memory:${key}`, `[TN Memory: ${key}]\n${ctx}`);
      }
    } catch {
      /* skip */
    }
...
const TOOLS = [
    ...
    inputSchema: {
      ...
      properties: {
        ...
        tags: {
          type: "array",
          items: { type: "string" },
          description: "Tags for categorizing this memory (e.g., deployment, bug-fix, architecture).",
        },
        category: {
          type: "string",
          description: "Memory category (e.g., decision, insight, failure, convention).",
        },
      },
      required: ["content"],
    },
  },
  {
    ...
    inputSchema: {
      type: "object",
      properties: {
        query: { type: "string", description: "Search query (keyword, concept, or tag prefix)." },
  ...
  },
  {
    ...
    inputSchema: {
      type: "object",
      properties: {
        query: { type: "string", description: "Describe what you need to do (e.g., 'browser automation' or 'Jira issue')." },
        limit: { type: "number", description: "Max results (default 10)." },
      },
      required: ["query"],
    },
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        tool: { type: "string", description: "Filter by source tool (claude-code, aider, gemini)." },
      },
      required: ["query"],
    },
  },
    ...
    inputSchema: {
      ...
      properties: {
        action: {
          ...
          description: "Action: list available skills, search by keyword, or install a skill.",
        },
        query: { type: "string", description: "Search query for skill discovery." },
        skill_id: { type: "string", description: "Skill ID to install (for install action)." },
      },
      required: ["action"],
    },
  },
  {
    ...
    inputSchema: {
      type: "object",
      properties: {
        query: { type: "string", description: "Code search query (pattern, symbol, or concept)." },
        scope: {
          type: "string",
          enum: ["ast-grep", "deepcontext", "file-pattern"],
          description: "Search scope: structural (AST), semantic (deepcontext), or file pattern.",
        },
      },
      required: ["query"],
    },
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        prompt: {
          type: "string",
          description: "Current task description to find relevant past context for.",
        },
        limit: { type: "number", description: "Max memories to harvest (default 10)." },
      },
      required: ["prompt"],
    },
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        key: { type: "string", description: "Scratchpad key (e.g., 'current_task', 'persona')." },
  ...
  },
  {
    ...
    inputSchema: {
      type: "object",
      properties: {
        action: { type: "string", description: "Action taken (e.g., 'bash', 'file_write', 'tool_call')." },
        target: { type: "string", description: "Target of the action (file path, tool name, URL)." },
        result: { type: "string", description: "Result or outcome of the action." },
      },
...
const SLASH_COMMANDS = {
  "tn-store": async (args) => {
    ...
    const tags = parts[1]
      ? parts[1].replace("]", "").split(",").map((t) => t.trim())
      : [];
  ...
  "tn-plan": async (args) => {
    ...
    await tnOk("/api/memory/add", {
      method: "POST",
      body: JSON.stringify({ content: args || "New plan", tags: ["plan"], category: "plan" }),
    });
  ...
  "tn-purge": async (args) => {
    ...
    await tnOk(`/api/memory/purge?tag=${encodeURIComponent(args)}`, { method: "POST" });
...
module.exports = {
  ...
  async onAuthorizeTool(toolName, args) {
    ...
    const isDangerous = dangerous.some(
      (d) => toolName.toLowerCase().includes(d) || JSON.stringify(args).toLowerCase().includes(d)
    );
    ...
    try {
      const r = await tnFetch("/api/commercial/authorize", {
        method: "POST",
        body: JSON.stringify({ tool: toolName, args: JSON.stringify(args).slice(0, 500) }),
      });
      const d = await r.json();
      return d.allowed !== false ? { allowed: true } : { allowed: false, reason: d.reason ?? "Blocked by commercial RBAC" };
    } catch {
  ...
  async activate(context) {
    ...
    return { ok: true, tools: TOOLS.length, commands: Object.keys(SLASH_COMMANDS).length };
  },
};
