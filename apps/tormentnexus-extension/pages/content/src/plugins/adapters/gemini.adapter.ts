...
export class GeminiAdapter extends BaseAdapterPlugin {
  ...
  private readonly fallbackSelectors = {
    ...
    SUBMIT_BUTTON:
      'button.mat-mdc-icon-button.send-button, button[aria-label*="Send"], button[data-testid="send-button"]',
    ...
    return new Promise((resolve, reject) => {
      ...
      const checkReady = () => {
        ...
        if (insertionPoint) {
          ...
          this.context.logger.debug(
            `Button insertion point not found, retrying... (attempt ${attempts}/${maxAttempts})`,
          );
