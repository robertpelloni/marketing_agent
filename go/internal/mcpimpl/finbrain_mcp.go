...
func HandleGetQuote_finbrain_mcp(ctx context.Context, args map[string]interface{}) (ToolResponse, error) {
...
func HandleGetNews_finbrain_mcp(ctx context.Context, args map[string]interface{}) (ToolResponse, error) {
	symbol, _ :=getString(args, "symbol")
	token, _ :=getString(args, "api_token")
	if symbol == "" || token == "" {
		return err("symbol and api_token required")
