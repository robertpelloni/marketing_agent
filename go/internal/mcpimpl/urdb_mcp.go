...
func HandleQuery_urdb_mcp(ctx context.Context, args map[string]interface{}) (ToolResponse, error) {
...
func HandleListDatabases_urdb_mcp(ctx context.Context, args map[string]interface{}) (ToolResponse, error) {
	baseURL := os.Getenv("URDB_BASE_URL")
	if baseURL == "" {
		baseURL = "http://localhost:8000"
	}
