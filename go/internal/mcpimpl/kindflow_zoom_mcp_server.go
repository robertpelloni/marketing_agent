...
func HandleListMeetings_kindflow_zoom_mcp_server(ctx context.Context, args map[string]interface{}) (ToolResponse, error) {
...
func HandleGetUser_kindflow_zoom_mcp_server(ctx context.Context, args map[string]interface{}) (ToolResponse, error) {
	userId, _ :=getString(args, "user_id")
	accessToken, _ :=getString(args, "access_token")
	if userId == "" || accessToken == "" {
		return err("user_id and access_token required")
