...
export class T3ChatAdapter extends BaseAdapterPlugin {
  ...
  private readonly selectors = {
    ...
    CHAT_INPUT:
      'textarea#chat-input, textarea[placeholder*="Type your message"], textarea.resize-none, div[contenteditable="true"]',
    // Submit button selectors (multiple fallbacks)
    SUBMIT_BUTTON:
      'button[type="submit"], button[aria-label*="Submit"], button[aria-label*="Send"], button.send-button',
    ...
    BUTTON_INSERTION_CONTAINER:
      'div[aria-label="Message actions"], .message-actions, .chat-input-actions, .-mb-px.mt-2.flex.w-full.flex-row-reverse.justify-between',
  ...
  constructor() {
    ...
    console.debug(
      `[T3ChatAdapter] Instance #${this.instanceId} created. Total instances: ${T3ChatAdapter.instanceCount}`,
    );
  }
  ...
  async initialize(context: PluginContext): Promise<void> {
    ...
    if (this.currentStatus === 'initializing' || this.currentStatus === 'active') {
      this.context?.logger.warn(
        `T3Chat adapter instance #${this.instanceId} already initialized or active, skipping re-initialization`,
      );
      return;
    }
  ...
  async insertText(text: string, options?: { targetElement?: HTMLElement }): Promise<boolean> {
    this.context.logger.debug(
      `Attempting to insert text into T3Chat chat input: ${text.substring(0, 50)}${text.length > 50 ? '...' : ''}`,
    );
